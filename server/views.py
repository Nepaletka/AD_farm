...
# Глобальный словарь для хранения информации о запущенных процессах
running_processes = {}
# Хранилище логов для каждого процесса (последние 1000 строк)
process_logs = {}

# Максимальное количество строк лога для хранения
MAX_LOG_LINES = 1000
...
# Временно отключено
# @app.route('/api/script_logs/<filename>')
# @auth.api_auth_required
# def get_script_logs(filename):
...
#         print(f"Getting logs for {safe_filename}, since: {since}, limit: {limit}")
        
#         if safe_filename not in process_logs:
#             print(f"No logs found for {safe_filename}")
#             return jsonify({"logs": [], "has_more": False, "total_count": 0})
        
#         logs = list(process_logs[safe_filename])
#         print(f"Found {len(logs)} total logs for {safe_filename}")
        
#         # Фильтруем по времени
#         if since > 0:
#             logs = [log for log in logs if log['timestamp'] > since]
#             print(f"Filtered to {len(logs)} logs since {since}")
...
#         print(f"Returning {len(logs)} logs, has_more: {has_more}")
        
#         return jsonify({
#             "logs": logs,
#             "has_more": has_more,
#             "total_count": len(process_logs[safe_filename])
#         })
        
#     except Exception as e:
#         print(f"Error getting logs for {filename}: {str(e)}")
#         return jsonify({"error": str(e)}), 500

# @app.route('/api/clear_script_logs/<filename>', methods=['DELETE'])
# @auth.api_auth_required
# def clear_script_logs(filename):
...
#         if safe_filename in process_logs:
#             process_logs[safe_filename].clear()
#             print(f"Cleared logs for {safe_filename}")
            
#         return jsonify({"message": f"Logs for '{safe_filename}' cleared successfully"})
